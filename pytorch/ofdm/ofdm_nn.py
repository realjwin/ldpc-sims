...
from nn.llr import LLRestimator, LLRestimator_withSNR
...
def train_nn_withSNR(input_samples, output_samples, data_timestamp, snrdb_low, snrdb_high, learning_rate, qbits, clipdb, ofdm_size, num_epochs, batch_size, load_model=None):
    #--- VARIABLES ---#
    ...
    if torch.cuda.device_count() > 1:
        ...
        LLRest = LLRestimator_withSNR(ofdm_size)
    ...
    filename = ts.strftime('%Y%m%d-%H%M%S') + '_qbits={}_clipdb={}_snrlow={}_snrhigh={}_lr={}.pth'.format(qbits, clipdb, snrdb_low, snrdb_high, learning_rate)
    filepath = 'outputs/model/' + filename
...
def train_nn_tanh(input_samples, output_samples, data_timestamp, snrdb_low, snrdb_high, learning_rate, qbits, clipdb, ofdm_size, num_epochs, batch_size, load_model=None):
    #--- VARIABLES ---#
    ...
    if torch.cuda.device_count() > 1:
        ...
        LLRest = LLRestimator_tanh(ofdm_size)
    ...
    for epoch in range(0, num_epochs):
        ...
        if np.mod(epoch, 10) == 0:
            with torch.no_grad():
                ...
                y_est_test = LLRest(x_test)
                test_loss = weighted_mse(y_est_test, y_test, 1e-1)
