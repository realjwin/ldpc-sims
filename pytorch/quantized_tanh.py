...
from ofdm.ofdm_nn import train_nn_tanh
...
results = '20191219-021144_tx=20191213-234355_unquantized_withsnr'
...
with open(results_path, 'rb') as f:
    ...
    pretrained_model = data['filenames'][0]
    snrdb_low = data['snrdb_low']
    snrdb_high = data['snrdb_high']
    
#redo snrdb_low and snrdb_high to the range I want
snrdb_low = 20
snrdb_high = 30
...
for qbits_idx, qbits_val in enumerate(qbits):
    for clipdb_idx, clipdb_val in enumerate(clipdb):
        ...
        filename = train_nn_tanh(input_samples, output_samples, timestamp, snrdb_low, snrdb_high, lr, qbits_val, clipdb_val, ofdm_size, num_epochs, batch_size, pretrained_model)
